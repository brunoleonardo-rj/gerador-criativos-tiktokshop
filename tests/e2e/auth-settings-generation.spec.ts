...
test("login, configuração, geração e cópia do VEO preservam a copy renderizada", async ({ page, context }) => {
  ...
  await page.getByText("e2e-creative-1").click();
  await expect(page.getByRole("heading", { name: "Prompt VEO 3" })).toBeVisible();
  await expect(page.getByText(/Fala: Eu uso este produto todos os dias/)).toBeVisible();
  const copyVeo = page.getByRole("button", { name: "Copiar Prompt VEO 3" });
  await copyVeo.click();
