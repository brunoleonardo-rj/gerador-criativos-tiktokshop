...
class MemoryStorage implements LibraryStorage {
  ...
  async promote(id: string) { const value = this.staged.get(id); if (!value) throw new Error("missing"); this.active.set(id, value.corpus); return { workbookPath: `versions/${id}.xlsx`, jsonPath: `versions/${id}.json` }; }
  async readJson(path: string) { const id = path.split("/").at(-1)!.replace(".json", ""); return this.active.get(id) ?? this.staged.get(id)!.corpus; }
  async verify(path: string, sha: string) { const id = path.split("/").at(-1)!.replace(".xlsx", ""); const value = this.active.get(id) ?? this.staged.get(id); return !!value && (this.staged.has(id) ? createHash("sha256").update(this.staged.get(id)!.workbook).digest("hex") === sha : true); }
  async remove() {} async cleanupStaged() {}
}
class MemoryRepository implements LibraryRepository {
  ...
  async activate(id: string, promoted: { workbookPath: string; jsonPath: string }, now: Date) { const staged = this.staged.get(id); if (!staged) throw new Error("missing"); const old = this.active; if (old) this.previous = { ...old, status: "PREVIOUS" }; this.active = { ...staged, ...promoted, status: "ACTIVE", activatedAt: now }; this.staged.delete(id); return { active: this.active, obsolete: [] }; }
...
describe("LibraryService", () => {
  ...
  it("activates a staged version and rolls back to the previous version", async () => {
    const storage = new MemoryStorage(); const repo = new MemoryRepository(); const first = corpus("1"); const firstBytes = Buffer.from("first"); storage.active.set("first", first); repo.active = { id: "first", sourceFilename: "first.xlsx", sourceSha256: createHash("sha256").update(firstBytes).digest("hex"), recordCount: 1, workbookPath: "versions/first.xlsx", jsonPath: "versions/first.json", status: "ACTIVE", validationSummary: {}, createdAt: new Date(), activatedAt: new Date() };
