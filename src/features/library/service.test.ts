...
class MemoryStorage implements LibraryStorage {
  ...
  failRemove = false;
  failRestore = false;
  async restoreStaged() { if (this.failRestore) throw new Error("restore"); }
...
class MemoryRepository implements LibraryRepository {
  ...
  async reconcileStagedPaths(id: string, promoted: { workbookPath: string; jsonPath: string }) { const staged = this.staged.get(id); if (staged) this.staged.set(id, { ...staged, ...promoted }); }
...
describe("LibraryService", () => {
  ...
  it("activates a staged version and rolls back to the previous version", async () => {
    const storage = new MemoryStorage(); const repo = new MemoryRepository(); const firstBytes = Buffer.from("first"); const first = { ...corpus("1"), sourceSha256: createHash("sha256").update(firstBytes).digest("hex") }; storage.active.set("first", first); repo.active = { id: "first", sourceFilename: "first.xlsx", sourceSha256: first.sourceSha256, jsonSha256: "a".repeat(64), recordCount: 1, workbookPath: "versions/first.xlsx", jsonPath: "versions/first.json", status: "ACTIVE", validationSummary: {}, createdAt: new Date(), activatedAt: new Date() };
