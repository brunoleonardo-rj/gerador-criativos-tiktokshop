...
class InMemorySettingsRepository implements SettingsRepository {
  ...
  async update(input: { encryptedApiKey?: EncryptedSecret; apiKeyLastFour?: string; model: string; veoTemplate: string }) {
    const current = await this.getOrCreate("{{copy_completa}}");
    this.record = {
      ...current,
...
describe("SettingsService", () => {
  ...
  it("só descriptografa a chave para configurações de geração", async () => {
    ...
    expect(await service.getGenerationSettings()).toMatchObject({ apiKey: "sk-ant-1234567890", model: "claude-sonnet-5" });
  });
  ...
  it("não salva um template com variáveis desconhecidas", async () => {
    ...
    await expect(
      service.update({ model: "claude-sonnet-5", veoTemplate: "{{variavel_inventada}}" }),
